...
/**
 * Default bounded agentic loop (D19). Opens one [[LlmSession]] per run (injecting the terminal `submit` tool whose
 * input_schema is the output schema), then drives it with ONLY the incremental messages each turn — the initial prompt,
 * then tool results, then any re-prompt feedback. It never re-sends accumulated context: the session owns the history.
 * Repeats until a schema-valid `submit` or the LoopPolicy budget is hit — then fails with [[AgenticRunFailed]] (never
 * returns an unstructured result). Performs no mutation itself; tools own any effects.
 */
final class DefaultAgenticLlmRunner[F[_]: Sync: UUIDGen](provider: LlmProvider[F]) extends AgenticLlmRunner[F] {
  ...
  private def loop[A](ctx: RunContext[F], state: LoopState)(using sc: StructuredCodec[A]): F[AgenticResult[A]] =
    if (state.iteration >= ctx.policy.maxIterations) {
      ...
      ctx.session.exchange(state.pending).flatMap { turn =>
  ...
  private def completeOrRetry[A](
    ...
    sc.decoder.decodeJson(submitCall.arguments) match {
      ...
      case Left(failure) =>
        loop(ctx, state.advanced(List(schemaRetryMessage(failure.getMessage)), recorded))
    }

  /** No `submit` yet: run every tool the model called, send the results back as the next delta, and continue. */
  ...
  private def toolResultMessages(results: List[ToolResult]): List[ChatMessage] =
    results.map(r => ChatMessage("tool", r.content.noSpaces))
...
object DefaultAgenticLlmRunner {
  ...
  /** Per-iteration loop state: the next delta to send, how many turns have elapsed, and the transcript so far. */
  final private case class LoopState(pending: List[ChatMessage], iteration: Int, transcript: Vector[Turn]) {

    def advanced(nextPending: List[ChatMessage], completed: Turn): LoopState =
      LoopState(nextPending, iteration + 1, transcript :+ completed)
